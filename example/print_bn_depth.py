...
class PrintDepthStrategy(StrategyTemplate):
    '''
...
def main(
    ...
    # 2. 创建 策略实例
    strategy = PrintDepthStrategy(name=name, subscribe_symbols={
        GatewayName.BINANCE_UM.value: subscribe_symbols
    })
    strategy.set_topic(gateway_name=GatewayName.BINANCE_UM.value, event_type=EventType.DEPTH, params={})
    # 3. 创建 MainEngine
